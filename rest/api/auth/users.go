...
func (handler *AppHandler) DemoAccount(c *gin.Context) {
	...
	if status == http.StatusOK {
		if baseProjectID != "" {
			...
			if err != nil {
				handler.Context.Logger.Errorf("Failed to add user to default group: %v", err)
			}
		}
...
func (handler *AppHandler) GetDemoProjectID() string {
	...
	if err != nil {
		handler.Context.Logger.Errorf("Failed to find demo project: %v", err)
		return ""
	}

	id, ok := project["_id"].(primitive.ObjectID)
	if !ok {
		handler.Context.Logger.Errorf("Invalid project ID: %v", err)
		return ""
	}
...
func (handler *AppHandler) GetDemoGroupID(projectID string) primitive.ObjectID {
	...
	if err != nil {
		handler.Context.Logger.Errorf("Failed to find default group: %v", err)
		return primitive.NilObjectID
	}

	id, ok := group["_id"].(primitive.ObjectID)
	if !ok {
		handler.Context.Logger.Errorf("Invalid group ID: %v", err)
		return primitive.NilObjectID
	}
